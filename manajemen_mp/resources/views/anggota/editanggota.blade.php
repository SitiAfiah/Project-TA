                ...
                <form action="{{ route('anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card border-0 shadow-sm mb-4" style="border-radius: 20px;">
                        <div class="card-header bg-white border-0 pt-4 px-4">
                            <h5 class="fw-bold text-primary mb-0"><i class="icon-user me-2"></i>Informasi Akun & Pribadi
                            </h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <small class="text-muted" style="font-size: 11px;">* Email digunakan untuk login
                                        sistem</small>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select name="role_id" class="form-select" required>
                                        @foreach ($data_role as $role)
                                            ...
                                            <option value="{{ $role->id }}"
                                                {{ $anggota->roles->contains('id', $role->id) ? 'selected' : '' }}>
                                                {{ $role->nama_role }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="jabatan" id="jabatanSelect" class="form-select border-primary"
                                        ...
                                        <option value="anggota"
                                            {{ !$anggota->roles->contains('nama_role', 'Pelatih') && !$anggota->roles->contains('nama_role', 'Pengurus') ? 'selected' : '' }}>
                                            Anggota</option>

                                        {{-- HANYA PENGURUS YANG BISA JADIKAN PELATIH/PENGURUS --}}
                                        @if ($userRole == 'Pengurus')
                                            <option value="pelatih"
                                                {{ $anggota->roles->contains('nama_role', 'Pelatih') ? 'selected' : '' }}>
                                                Pelatih</option>
                                            <option value="pengurus"
                                                {{ $anggota->roles->contains('nama_role', 'Pengurus') ? 'selected' : '' }}>
                                                Pengurus</option>
                                        @endif
                                    </select>

                                    @if ($userRole != 'Pengurus')
                                        <small class="text-info mt-1 d-block"><i class="icon-info me-1"></i>Anda hanya dapat
                                            mengubah data Anggota biasa.</small>
                                    @endif
                                </div>
                                <div class="col-md-4">
                        ...
                        <div class="card-footer bg-light border-0 p-4" style="border-radius: 0 0 20px 20px;">
                            <div class="d-flex justify-content-end gap-3">
                                <a href="{{ route('anggota.anggota') }}"
                                    class="btn btn-outline-secondary px-4 fw-bold btn-cancel">Batal</a>
                                <button type="submit" class="btn btn-primary px-5 fw-bold btn-save shadow">Simpan
                                    Perubahan</button>
                            </div>
                        </div>
                    </div>
